...
ExecuteInGameThread(function()
	...
	if dt_dungeons_room_enemies_set and dt_dungeons_room_enemies_set:IsValid() then
...
NotifyOnNewObject("/Game/Blueprint/Enemy/BP_SevTargetPoint_EnemySet.BP_SevTargetPoint_EnemySet_C", function(target_object)
	if target_object:IsValid() then
		...
		local function patch()
			-- local path_target_object = StaticFindObject(target_path)
			local path_target_object = target_object
			if not(path_target_object:IsValid()) or path_target_object.mEnemySetID:ToString() == "None" then
				print("まだ初期化前？")
			...
			print("パッチ実行:" .. path_target_object.mEnemySetID:ToString())
			path_target_object.mEnemySetID = FName("TEST_TestZombies")
		end
		ExecuteWithDelay(1, patch)
	end
end)
